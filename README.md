...
Damage falls off toward the edge of a blast, and a piece that hits something bursts there without splitting further. **Each burst follows the damage its piece does**: the blast radius and the crater it bites out of rock both scale with the square root of the piece's strength, so their area matches the damage (a full-strength shell has an 85-unit blast and a 48-unit crater; a generation-two piece about 38 and 22, a generation-four piece about 17 and 10), and it shoves loose rock in proportion too. In the test (`-fractaltest`) one shell aimed at a pair of drones 600 units apart did nearly **3 times the damage of a homing shell**, which can only find one of them, and one shell aimed a little short of a cluster cleared three ordinary drones. Aim it so it divides *before* it reaches the target: a shell that arrives whole is just a heavy shell.
