...
setlocal
set SRC=src\main.cpp src\game.cpp src\level.cpp src\level_hud.cpp src\enemies.cpp src\enemies_fx.cpp src\ships.cpp src\audio.cpp src\sounds.cpp src\audio_game.cpp src\audio_check.cpp src\weapons.cpp src\shop.cpp src\world.cpp src\render.cpp src\field.cpp src\gl.cpp
...
cl /nologo /std:c++17 /O2 /EHsc /DNDEBUG /D_CRT_SECURE_NO_WARNINGS /W3 %SRC% /Fe:build\asteroid.exe /Fo:build\ ^
   /link opengl32.lib gdi32.lib user32.lib winmm.lib
goto done

:gcc
echo Building with MinGW-w64...
g++ -std=c++17 -O3 -ffast-math -fno-math-errno -DNDEBUG -Wall -Wno-unused-variable ^
    %SRC% -o build\asteroid.exe -lopengl32 -lgdi32 -lwinmm -static
